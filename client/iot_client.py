...
class GreenhouseDashboard(ctk.CTk):
    ...
    def _handle_disconnection(self):
        """Atualiza a UI para refletir o estado offline e limpa o socket."""
        self.status_label.configure(text="OFFLINE", text_color="#ef4444")
        self._log("error", "Disconnected from Broker.")
        
        # Opcional: Zerar labels dos sensores para indicar que os dados não são mais confiáveis
        ...
        if self.sock:
            try:
                ...
                pass
        self.after(5000, self._connect_broker) # Tenta reconectar a cada 5 segundos
    ...
    def _connect_thread(self):
        ...
        try:
            ...
            self.connected = False
            self.after(0, self._handle_disconnection)
            self.after(0, lambda: self._log("error", f"Connection failed: {e}"))
    ...
    def _receive_loop(self):
        while self.connected:
            try:
                ...
                self.connected = False
                self.after(0, self._handle_disconnection)
                self.after(0, lambda: self._log("error", f"Receive Loop Exception: {e}"))
                break
