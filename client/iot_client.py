...
# BROKER_IP = os.environ.get("BROKER_IP", "172.16.103.10") 
...
class GreenhouseDashboard(ctk.CTk):
    def __init__(self):
        ...
        self.sensors = {
            ...
            "rh": random.uniform(50.0, 70.0),
            "lux": random.uniform(400.0, 700.0),
            "gas": random.uniform(20.0, 50.0)
        }
    ...
    def _connection_supervisor(self):
        ...
        self.connected = True
        self.after(0, lambda: self._update_status("ONLINE"))
        self.after(0, lambda: self._log("system", f"Connected to broker at {BROKER_IP}:{PORT}"))
        self.after(0, lambda: self._log("out", f"[CONNECT] Client ID: {CLIENT_ID}"))
        time.sleep(0.2)
        self.after(0, lambda: self._log("in", "[CONNACK] Session Accepted"))

        topics = ["greenhouse/temp", "greenhouse/humidity", "greenhouse/light", "greenhouse/gas"]
        for t in topics:
            self.after(0, lambda topic=t: self._log("out", f"[SUBSCRIBE] {topic}"))
            time.sleep(0.1)
        
        time.sleep(0.3)
        self.after(0, lambda: self._log("in", "[SUBACK] Subscriptions Confirmed"))

        threading.Thread(target=self._simulate_receive_loop, daemon=True).start()
        threading.Thread(target=self._simulate_ping_loop, daemon=True).start()

    def _simulate_receive_loop(self):
        topics_map = {
            "temp": ("greenhouse/temp", 15.0, 35.0),
            "rh": ("greenhouse/humidity", 30.0, 90.0),
            "lux": ("greenhouse/light", 100.0, 900.0),
            "gas": ("greenhouse/gas", 0.0, 80.0)
        }
        
        while self.connected:
            time.sleep(random.uniform(1.0, 3.5))
            
            key = random.choice(list(topics_map.keys()))
            topic, min_v, max_v = topics_map[key]
            
            change = random.uniform(-1.5, 1.5)
            new_val = max(min_v, min(max_v, self.sensors[key] + change))
            self.sensors[key] = new_val
            
            msg_dict = {"value": round(new_val, 2)}
            msg_str = json.dumps(msg_dict)
            
            self.after(0, lambda t=topic, m=msg_str: self._log("in", f"[{t}] {m}"))
            self.after(0, self._refresh_ui)

    def _simulate_ping_loop(self):
        while self.connected:
            time.sleep(15)
            self.after(0, lambda: self._log("out", "[PINGREQ] 0xC0 0x00"))
            time.sleep(0.1)
            self.after(0, lambda: self._log("in", "[PINGRESP] 0xD0 0x00"))
    ...
    def _publish(self, topic, payload):
        ...
        def simulate_actuator_response():
            ...
            self.after(0, lambda: self._log("system", f"Actuator acknowledged: {topic}"))
            
        threading.Thread(target=simulate_actuator_response, daemon=True).start()
