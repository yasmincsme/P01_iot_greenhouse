...
# def build_connect_packet(client_id):
#     proto = "MQTT".encode('utf-8')
#     var_h = bytearray([0x00, 0x04]) + proto + bytearray([0x04, 0x02, 0x00, KEEP_ALIVE])
#     cid = client_id.encode('utf-8')
#     payload = bytearray([len(cid) >> 8, len(cid) & 0xFF]) + cid
#     rl_bytes = encode_remaining_length(len(var_h) + len(payload))
#     return bytearray([0x10]) + rl_bytes + var_h + payload
...
#                 time.sleep(3)
#             else:
#                 logging.warning(f"[{CLIENT_ID}] 20s elapsed. Entering idle mode to trigger timeout...")
#                 time.sleep(40)
            
#     except Exception as e:
#         logging.warning(f"[{CLIENT_ID}] Connection Lost/Closed: {e}")
#     finally:
#         sock.close()
